...
(function ($) {
	...
	window.jPrompt = function (msg, bb,default_value, callback, title) {
		...
		var tpl = '<div>' +
			'<div>' + msg + '</div>' +
			'<div>' +
			'<input autofocus id="rq" class="xdsoft_prompt" type="date" value="' + (default_value ? default_value.replace(/[&<>"']/g, function (match) {return '&#' + match.charCodeAt(0) + ';'; }) : '') + '">' +
			'</div>' +
			'<div>' + bb + '</div>' +
			'<div>' +
			'<input autofocus id="bbh" class="xdsoft_prompt" type="text" value="' + (default_value ? default_value.replace(/[&<>"']/g, function (match) {return '&#' + match.charCodeAt(0) + ';'; }) : '') + '">' +
			'</div>' +
			'</div>';
		...
		return $(tpl)[plugin_name]({
			...
			buttons: {
				'确定': function (event) {
					var val = this.find('#rq').val();
					var val1 = this.find('#bbh').val();
					(callback || function () {}).call(this, event, val,val1);
				},
				'取消': true
			}
		});
