...
<script src="../../assets/js/xschange/rjwt.js"></script>
...
<script type="text/javascript">
	...
	window.onload = function() {
		...
		gethzunitname();
		dwcx("问题管理子页面单位查询", gethy, getzy, getrjmc, "软件问题");
		var option4 = $("#wtcause option:selected");
		var option5 = $(".active").find("a").text().substring(0,3);
		var option6 = $("#dwcx option:selected");
	//	alert(option4.text()+"......"  +option5 +"......."+ option6.text());
		wentiquery("问题查询按钮1", gethy,getzy, getrjmc, option4.text(), option5, option6.text(),"软件问题");
	};
	...
	$(window.parent.frames["middleTopFrame"].document).find("#rjmc").change(function() {
						if (getrjmc != "请选择") {
							...
							queryDataBarAndPie("软件问题图表数据查询", gethy, getzy, getrjmc, "软件问题");
							barshow();
							queryDataBarAndPie("子页面图表数据查询pie1", gethy, getzy, getrjmc, "软件问题");
							pieshow1();
							queryDataBarAndPie("子页面图表数据查询pie", gethy, getzy, getrjmc, "软件问题");
							pieshow();
						} else {
							gethzunitname();
							 getrjmc1 = [ getrjmc ];
	...
	var tableToExcel = (function() {
		...
		return function(table, filename, sheetname) {
			var a = $('<a id="dlink" style="display:none;"/>');	 //这里创建一个<a/>标记利用a标记的download属性来自定义Excel文件名,解决button按钮触发下载无法自定义Excel文件名的问题
			...
			a.attr("download", '软件问题表'); //这里是关键所在,当点击之后,设置a标签的属性,这样就可以更改excel文件的名字了
		...
		<div class="xschange" id="wtcx">
			<form method="POST" class="form-horizontal" role="form" name="form"
				...
				<div id="toolbox" style="display:block;margin-top:-20px;">
					...
					<div class="selc">
						<span id="slelecthead">问题发生原因:</span> 
						<span style="margin-right:25px;"> 
							<select id="wtcause" class="selectway" name="wtcause">
								<option>请选择</option>
							</select>
						</span>
					</div>
...
<script type="text/javascript">
	$("#allsoft").click(function() {
		//queryDataBarAndPie("图表数据查询", gethy, getzy, getrjmc, "软件问题");
	});
