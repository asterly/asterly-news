...
<script src="../../assets/js/xschange/yhsy.js"></script>
...
var getrjmc;//字符串形式
	window.onload = function() {
		...
		gethzunitname();
		dwcx("问题管理子页面单位查询", gethy, getzy, getrjmc, "用户使用");
		var option4 = $("#wtcause option:selected");
		var option5 = $(".active").find("a").text().substring(0, 3);
		var option6 = $("#dwcx option:selected");
		wentiquery("问题查询按钮1", gethy, getzy, getrjmc, option4.text(), option5, option6.text(), "用户使用");
	};
	...
	$(window.parent.frames["middleTopFrame"].document).find("#rjmc").change(function() {
						if (getrjmc != "请选择") {
							...
							queryDataBarAndPie("软件问题图表数据查询", gethy, getzy, getrjmc, "用户使用");
							barshow();
							queryDataBarAndPie("子页面图表数据查询pie1", gethy, getzy, getrjmc, "用户使用");
							pieshow1();
							queryDataBarAndPie("子页面图表数据查询pie", gethy, getzy, getrjmc, "用户使用");
							pieshow();
						} else {
							gethzunitname();
							 getrjmc1 = [ getrjmc ];
		...
		$.ajax({
			...
			success : function(response) {
				...
				for (var i = 0, len = tlj.length; i < len; i++) {
					tljSelectStr +="<option id='" + tlj[i] + "'>" + tlj[i] + "</option>";
				}
	...
	var tableToExcel = (function() {
		...
		return function(table, filename, sheetname) {
			...
			a.attr("download", '用户使用表'); //这里是关键所在,当点击之后,设置a标签的属性,这样就可以更改excel文件的名字了
	...
	function educe(){
		...
		var option6 = $("#dwcx option:selected");
		var fslx = "用户使用";
		wentiquery1("问题查询按钮1", gethy, getzy, getrjmc, option4.text(), option5, option6.text(), fslx);
		var c=document.getElementById("dlink");//此处是对表格导出方法中，append（标签的判断），如果存在，则进行移除，
		if(c!=null){
			c=	c.remove();
		}
		exportToExcel('tab', '用户使用', '用户使用表');
...
<body>
	...
	<div id="right">
		<div id="Modal1" class="modal">
			<span id="close1" class="close1 cursor">&times;</span>
					...
					<table id="tab2">
						<thead style="background:#ADD8E6;">
							<tr>
								<th width="4%;">序号</th>
								<th width="7%;">单位</th>
								<th width="5%;">上级单位</th><!-- /原来下级单位八月十一日修改 -->
								<th width="5%;">部门</th>
								<th width="5%;">反馈人</th>
								<th width="5%;">联系电话</th>
								<th width="7%;">软件</th>
		...
		<div id="myModal" class="modal">
			<span id="close2" class="close2 cursor">&times;</span>
		...
		<div class="xschange" id="wtcx">
			<form method="POST" class="form-horizontal" role="form" name="form"
				...
				<div id="toolbox" style="display:block;margin-top:-20px;">
					...
					<div class="selc">
						<span id="slelecthead">单位:</span> <span style="margin-right:25px;">
			...
			<div id="btns" style="display:block">
				...
				<input type="button" class="btn btn-primary btn-sm" id="selectso" value="已解决" />
			...
			<div id="myTabContent" class="tab-content">
				...
				<div class="tab-pane fade in" id="zsolve">
					<ul id="myTabData" class="nav nav-tabs" style="display: none">
						<li class="active"><a href="#allpo" data-toggle="tab" id="allsoft">总体分析</a>
						</li>
					...
					<div id="myTabContent1" class="tab-content">
						...
						<!-- 按软件问题类型 -->
						<!-- <div class="tab-pane fade in" id="qupo">												
							<div id="datatj" style="width:100%;margin-top:40px;">
								<div id="barshow" style="width:50%;">
									<div id="zchartmain" style="width: 500px; height: 450px;"></div>
								</div>
								
								<div id="pieshow" style="width:50%;">
									<div id="bchartmain" style="width: 500px; height: 450px;"></div>
								</div>
							</div>
						
						</div> -->
						<!-- 按单位名称 -->
						<!-- <div class="tab-pane fade in" id="unpo">												
							<div id="datatj" style="width:100%;margin-top:40px;">
								<div id="barshow" style="width:50%;">
									<div id="uzchartmain" style="width: 500px; height: 450px;"></div>
								</div>
								
								<div id="pieshow" style="width:50%;">
									<div id="ubchartmain" style="width: 500px; height: 450px;"></div>
								</div>
							</div>
...
<script type="text/javascript">
	$("#allsoft").click(function() {
		//queryDataBarAndPie("图表数据查询", gethy, getzy, getrjmc, "用户使用");
	});
	...
	/* $("#qsoft").click(function() {
		...
		$("#btns").css("display", "none");
			...
			queryDataBarAndPie("图表数据查询", gethy, getzy, getrjmc, "用户使用");
			barshow();
			queryDataBarAndPie("图表数据查询pie1", gethy, getzy, getrjmc, "用户使用");
			pieshow1();
			queryDataBarAndPie("图表数据查询pie", gethy, getzy, getrjmc, "用户使用");
			pieshow();
	}) */
