...
<script src="../../assets/js/xschange/gcaz.js"></script>
...
var getrjmc;//字符串形式
	window.onload = function() {
		...
		gethzunitname();
		dwcx("问题管理子页面单位查询", gethy, getzy, getrjmc, "工程安装");
		var option4 = $("#wtcause option:selected");
		var option5 = $(".active").find("a").text().substring(0,3);
		var option6 = $("#dwcx option:selected");
		wentiquery("问题查询按钮1", gethy, getzy, getrjmc, option4.text(), option5, option6.text(), "工程安装");
	};
	...
	$(window.parent.frames["middleTopFrame"].document).find("#rjmc").change(function() {
						if (getrjmc != "请选择") {
							...
							queryDataBarAndPie("软件问题图表数据查询", gethy, getzy, getrjmc, "工程安装");
							barshow();
							queryDataBarAndPie("子页面图表数据查询pie1", gethy, getzy, getrjmc, "工程安装");
							pieshow1();
							queryDataBarAndPie("子页面图表数据查询pie", gethy, getzy, getrjmc, "工程安装");
							pieshow();
						} else {
							gethzunitname();
							 getrjmc1 = [ getrjmc ];
	...
	var tableToExcel = (function() {
		...
		return function(table, filename, sheetname) {
			...
			a.attr("download", '工程安装表'); //这里是关键所在,当点击之后,设置a标签的属性,这样就可以更改excel文件的名字了
...
<script type="text/javascript">
	$("#allsoft").click(function() {
		//queryDataBarAndPie("图表数据查询", gethy, getzy, getrjmc, "工程安装");
	});
	...
	/* $("#qsoft").click(function() {
		...
		$("#btns").css("display", "none");
			...
			queryDataBarAndPie("图表数据查询", gethy, getzy, getrjmc, "工程安装");
			barshow();
			queryDataBarAndPie("图表数据查询pie1", gethy, getzy, getrjmc, "工程安装");
			pieshow1();
			queryDataBarAndPie("图表数据查询pie", gethy, getzy, getrjmc, "工程安装");
			pieshow();
	}) */
